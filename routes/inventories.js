...
router.get("/inventories/:id", (req, res) => {
  ...
  knex("inventories")
    ...
    .catch((err) =>
      res.status(400).send(`Error retrieving Inventory ${id}: ${err}`)
    );
});
// -----------------SEYON CODE END----------------------------------------
...
router.post("/inventories", async (req, res) => {
  try {
    ...
    const { warehouse_id, item_name, description, category, status, quantity } =
      req.body;
    const warehouseIdCheck = await knex("warehouses").where({
      id: warehouse_id,
    });
    ...
    if (
      ...
      return res
        .status(400)
        .send("Please verify there are no blank input fields");
    } else if (!warehouseIdCheck.length) {
      return res
        .status(400)
        .send(`A warehouse with ID ${warehouse_id} does not exist`);
  ...
  knex("inventories")
    ...
    .then((count) => {
      if (count > 0) {
        res
          .status(200)
          .send(`Inventory item with id ${id} deleted successfully`);
      } else {
        res.status(404).send(`Inventory item with id ${id} not found`);
      }
    })
    .catch((err) =>
      res.status(400).send(`Error deleting Inventory item ${id}: ${err}`)
    );
});
// -----------------SEYON CODE END----------------------------------------
